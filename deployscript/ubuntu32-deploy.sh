...
# gcc-9 ppa:
echo "deb http://ppa.launchpad.net/ubuntu-toolchain-r/test/ubuntu ${CHROOT_DISTRO} main" >> /etc/apt/sources.list
echo "deb-src http://ppa.launchpad.net/ubuntu-toolchain-r/test/ubuntu ${CHROOT_DISTRO} main" >> /etc/apt/sources.list
...
apt install -y --install-recommends software-properties-common wget sudo make cmake libc-dev gcc-9 g++-9 xz-utils || die "* apt basic erro!"
